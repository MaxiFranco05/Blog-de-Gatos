...
def new_post(post: NewPost):
    with SessionLocal() as db:
        ...
        title_str = str(post.title) if post.title else ""
        img = post.profile_pic if hasattr(post, "profile_pic") and post.profile_pic else f"https://ui-avatars.com/api/?background=random&name={title_str.replace(' ', '-')}
